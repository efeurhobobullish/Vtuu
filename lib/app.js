...
app.post("/signup", async (req, res) => {
    try {
        const { username, email, password, validationCode } = req.body;

        // Check for missing fields
        if (!username || !email || !password || !validationCode) {
        ...
        const newUser = new User({
            ...
            validationCode,
            isEmailValidated: false, // Default: user not verified
        });
        ...
        const verificationLink = `https://testpay-61h6.onrender.com/verify-email?token=${token}`;
...
cron.schedule('0 * * * *', async () => { // Runs every hour
    const unverifiedUsers = await User.find({ isEmailValidated: false });
    unverifiedUsers.forEach(user => {
        // Send reminder email logic here
        sendEmail(user.user_email, "Reminder: Verify Your Email", "Please verify your email to activate your account.");
    });
});
...
app.get("/verify-email", async (req, res) => {
    try {
        ...
        console.log("User  before verification:", user); // Log user state
...
app.post('/resend-verification', async (req, res) => {
    ...
    res.status(400).json({ success: false, message: "User  not found or already verified." });
});
