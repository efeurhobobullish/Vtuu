...
async function connectDB() {
    try {
        ...
        // Ensure update runs **only after** the connection is established
...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const verificationLink = `https://empiretech.biz.id/verify-email?token=${token}`;
    ...
    await sendEmail(
      ...
      `<p>Click the button below to verify your email:</p>
       <a href="${verificationLink}" style="background: purple; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Verify Email</a>`
    );

    res.json({ success: true, message: "Signup successful! Please check your email for verification." });
  } catch (error) {
    console.error(error);
...
const sendEmail = async (to, subject, text, html) => {
  try {
    const mailOptions = {
      from: `"EmpirePay Vtu" <noreply@empiretech.biz.id>`,
      to,
      subject,
      text,
...
app.get("/api/verify-email", async (req, res) => {
    ...
    try {
        ...
        await User.updateOne({ email: decoded.email }, { $set: { verified: true } });
